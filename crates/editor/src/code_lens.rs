...
use crate::{
    Editor, EditorSettings, LSP_REQUEST_DEBOUNCE_TIMEOUT, MultibufferSelectionMode,
    SelectionEffects,
...
pub(super) struct CodeLensState {
    ...
    pub(super) enabled_in_settings: bool,
...
impl CodeLensState {
    pub(super) fn new(cx: &App) -> Self {
        let enabled_in_settings = EditorSettings::get_global(cx).code_lens.enabled();
        Self {
            enabled_in_settings,
...
fn default_client_command(command_name: &str) -> Option<ClientCommand> {
    match command_name {
...
fn try_show_references(
    ...
    let fallback_title = action.lsp_action.title().to_owned();
...
impl Editor {
    pub(super) fn refresh_code_lenses(
        ...
        let Some(code_lens) = self.code_lens.as_mut() else {
            return;
        };
        code_lens.refresh_task = cx.spawn(async move |editor, cx| {
            ...
            editor
                .update(cx, |editor, cx| {
                    let Some(code_lens) = editor.code_lens.as_mut() else {
                        return;
                    };
                    ...
                    for (buffer_id, blocks) in all_new_blocks {
                        ...
                        if let Some(code_lens) = editor.code_lens.as_mut() {
                            code_lens.block_ids.insert(buffer_id, block_ids);
                        }
                    }
    ...
    pub fn code_lens_enabled(&self) -> bool {
        self.code_lens.as_ref().is_some_and(|cl| cl.enabled)
    }
    ...
    pub(super) fn clear_code_lenses(&mut self, cx: &mut Context<Self>) {
        let all_blocks = self
            .code_lens
            .as_ref()
            .map(|cl| cl.all_block_ids())
            .unwrap_or_default();
        if !all_blocks.is_empty() {
            self.remove_blocks(all_blocks, None, cx);
        }
        if let Some(code_lens) = self.code_lens.as_mut() {
            code_lens.block_ids.clear();
            code_lens.refresh_task = Task::ready(());
            cx.notify();
        }
    }
}
...
mod tests {
    ...
    async fn test_code_lens_toggling(cx: &mut TestAppContext) {
        ...
        cx.update_editor(|editor, window, cx| {
            editor.toggle_code_lens(false, window, cx);
        });
        ...
        cx.editor.read_with(&cx.cx.cx, |editor, _cx| {
            assert_eq!(
                editor.code_lens_enabled(),
                false,
                "code lens should be disabled after toggling off"
            );
        });
    }
}
